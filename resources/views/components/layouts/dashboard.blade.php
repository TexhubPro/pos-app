...
<body class="min-h-full text-gray-900 antialiased bg-slate-50">
    ...
    <script>
        ...
        (() => {
            const toggles = document.querySelectorAll('[data-user-dropdown-toggle]');
            const menus = new Map();

            toggles.forEach(toggle => {
                const target = document.querySelector(toggle.getAttribute('data-user-dropdown-toggle'));
                if (!target) return;
                menus.set(toggle, target);

                const close = () => target.classList.add('hidden');
                const open = () => target.classList.remove('hidden');

                toggle.addEventListener('click', (e) => {
                    e.preventDefault();
                    target.classList.contains('hidden') ? open() : close();
                });
